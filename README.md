...
Starting from `v1.0.4`, the package also ships with a built-in skill, `basic-skill`, which helps agents combine these tools in a more reliable way.
...
Starting from `v1.0.4`, the package also includes an **OpenClaw skill**:
...
Starting from `v1.0.4`, the package ships with the `basic-skill` skill. It teaches the agent to use the tools in a short loop: observe -> act -> verify -> observe again. Make sure you installed a recent version and restarted the Gateway so the latest skill was loaded.
