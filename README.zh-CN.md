...
从 `v1.0.6` 开始，插件会一并发布内置 skill `basic-skill`，用于教 Agent 更稳定地组合使用这些工具。
...
从 `v1.0.6` 开始，仓库还会随包发布一个 **OpenClaw Skill**：
...
从 `v1.0.6` 开始，插件会随包发布 `basic-skill` skill。它会教 Agent 按“观察 -> 行动 -> 验证 -> 再观察”的闭环使用工具。请确认当前安装的是较新版本，并已重启 Gateway 让最新 skill 被加载。
