...
从 `v2026.3.27` 开始，插件会一并发布内置 skills（包含 `basic-skill`），用于教 Agent 更稳定地组合使用这些工具。
...
从 `v2026.3.27` 开始，仓库会随包发布 **OpenClaw Skills**：
...
从 `v2026.3.27` 开始，插件会随包发布内置 skills（如 `basic-skill`）。它们会教 Agent 按“观察 -> 行动 -> 验证 -> 再观察”的闭环使用工具。请确认当前安装的是较新版本，并已重启 Gateway 让最新 skills 被加载。
